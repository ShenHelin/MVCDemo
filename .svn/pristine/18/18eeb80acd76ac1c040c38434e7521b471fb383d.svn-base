...
<body>
	...
	<table border="1">
		...
		<tr>
			...
			<td>${product.price }</td>
			<td>
			<a href="${pageContext.request.contextPath }/product_delete.action?pid=${product.pid }" onClick="return confirm('确认删除吗？');">删除</a>
			<a href="${pageContext.request.contextPath }/product_find_findById.action?pid=${product.pid}&option=update">修改</a>
			</td>
		</tr>
		</c:forEach>
	</table>
	...
	<br/>
	<br/>
	<form method="post" action="${pageContext.request.contextPath }/product_find_findById.action">
	PID:<input type="number" name="pid" value="1">
	<input type="submit" value="精确查询">
	</form>
	<br/>
	<br/>
	<form method="post" action="${pageContext.request.contextPath }/product_find_findByName.action">
